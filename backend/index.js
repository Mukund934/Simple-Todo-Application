...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: foundUser._id, username: foundUser.username }, SECRET, { expiresIn: "1h" });
    res.json({ token });
  } catch (err) {
    console.error(err.stack);
...
app.post("/todo", authenticateToken, async (req, res) => {
  ...
  if (!parsedPayload.success) {
    return res.status(400).json({ msg: "Invalid input", error: parsedPayload.error.errors });
  }
  try {
    await todo.create({
      title: payload.title,
